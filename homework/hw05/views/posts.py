...
class PostListEndpoint(Resource):
    ...
    def post(self):
        ...
        if not image_url:
                return Response(
                    json.dumps({"Message": f"image_url is a required parameter."}),
                    mimetype="application/json",
                    status=400,
                )
        # 1. Create:
...
class PostDetailEndpoint(Resource):
    ...
    def patch(self, id):
        ...
        if post is None:
            return Response(
                json.dumps({"Message": "This post does not exist. Try again"}),
                mimetype="application/json",
                status=404,
                )
        ...
        if post.user_id != self.current_user.id:
            return Response(
                json.dumps({"Message": f"You are not authorized to modify post id= {id}"}),
                mimetype="application/json",
                status=404,
            )
    ...
    def delete(self, id):
        ...
        # 2. Check that the owner is the same person as the person logged in.
        if post.user_id == self.current_user.id:
            Post.query.filter_by(id=id).delete()
            db.session.commit()
            return Response(
            json.dumps({"Message": "The post has been successfully deleted."}),
            mimetype="application/json", 
            status=200)
        else:
            return Response(
                json.dumps({"Message": "You are not authorized to delete this post"}),
                mimetype="application/json",
                status=404,
                )
